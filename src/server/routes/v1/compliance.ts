...
complianceRouter.get('/consent-templates', authMiddleware, async (c) => {
	...
	try {
		// TODO: Temporariamente desabilitar verificação de tipo para permitir compilação
		...
		return c.json(
			{
				code: 'CONSENT_TEMPLATES_ERROR',
				error: 'Erro ao buscar modelos de consentimento',
			},
			500,
		);
	}
	...
	async (c) => {
		...
		try {
			...
			return c.json(
				{
					data: consent,
					meta: { requestId, grantedAt: new Date().toISOString() },
				},
				201,
			);
		} catch (error) {
