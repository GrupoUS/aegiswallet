...
export class ComplianceService {
  ...
  async getUserConsents(userId: string): Promise<LgpdConsent[]> {
    ...
    if (error) {
      secureLogger.error('Failed to get user consents', { error: error.message, userId });
  ...
  async getConsentTemplates(): Promise<ConsentTemplate[]> {
    ...
    if (error) {
      secureLogger.error('Failed to get consent templates', { error: error.message });
      throw new Error(`Erro ao buscar modelos de consentimento: ${error.message}`);
    }
  ...
  async grantConsent(
    ...
    if (error) {
      secureLogger.error('Failed to grant consent', { error: error.message, userId, consentType });
  ...
  async revokeConsent(userId: string, consentType: ConsentType): Promise<void> {
    ...
    if (error) {
      secureLogger.error('Failed to revoke consent', { error: error.message, userId, consentType });
  ...
  async createExportRequest(
    ...
    if (error) {
      secureLogger.error('Failed to create export request', { error: error.message, userId });
      throw new Error(`Erro ao criar solicitação de exportação: ${error.message}`);
    }
  ...
  async getExportRequests(userId: string): Promise<DataExportRequest[]> {
    ...
    if (error) {
      secureLogger.error('Failed to get export requests', { error: error.message, userId });
      throw new Error(`Erro ao buscar solicitações de exportação: ${error.message}`);
    }
  ...
  async createDeletionRequest(
    ...
    if (hasHold && hasHold.length > 0) {
      throw new Error('Seus dados estão sob retenção legal e não podem ser excluídos no momento.');
    }
    ...
    if (error) {
      secureLogger.error('Failed to create deletion request', { error: error.message, userId });
      throw new Error(`Erro ao criar solicitação de exclusão: ${error.message}`);
    }
  ...
  async getDeletionRequests(userId: string): Promise<DataDeletionRequest[]> {
    ...
    if (error) {
      secureLogger.error('Failed to get deletion requests', { error: error.message, userId });
      throw new Error(`Erro ao buscar solicitações de exclusão: ${error.message}`);
    }
  ...
  async getTransactionLimits(userId: string): Promise<TransactionLimit[]> {
    ...
    if (error) {
      secureLogger.error('Failed to get transaction limits', { error: error.message, userId });
  ...
  async checkTransactionLimit(
    ...
    if (error && error.code !== 'PGRST116') {
      secureLogger.error('Failed to check limit', { error: error.message, userId, limitType });
  ...
  async getAuditHistory(
    ...
    if (error) {
      secureLogger.error('Failed to get audit history', { error: error.message, userId });
      throw new Error(`Erro ao buscar histórico de auditoria: ${error.message}`);
    }
...
// Factory function
export function createComplianceService(supabase: SupabaseClientType): ComplianceService {
