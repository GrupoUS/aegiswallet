...
async function getBodyDimensions(page) {
  const bodyDimensions = await page.evaluate(() => {
    ...
    return {
      height: parseFloat(style.height), scrollHeight: body.scrollHeight, scrollWidth: body.scrollWidth, width: parseFloat(style.width)
    };
  });
  ...
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      targetSlide.addImage({
        h: el.position.h, path: imagePath, w: el.position.w, x: el.position.x, y: el.position.y
      });
    } else if (el.type === 'line') {
      targetSlide.addShape(pres.ShapeType.line, {
        h: el.y2 - el.y1, line: { color: el.color, width: el.width }, w: el.x2 - el.x1, x: el.x1, y: el.y1
      });
    } else if (el.type === 'shape') {
      const shapeOptions = {
        h: el.position.h, shape: el.shape.rectRadius > 0 ? pres.ShapeType.roundRect : pres.ShapeType.rect, w: el.position.w, x: el.position.x, y: el.position.y
      };
      ...
      const listOptions = {
        align: el.style.align, color: el.style.color, fontFace: el.style.fontFace, fontSize: el.style.fontSize, h: el.position.h, lineSpacing: el.style.lineSpacing, margin: el.style.margin, paraSpaceAfter: el.style.paraSpaceAfter, paraSpaceBefore: el.style.paraSpaceBefore, valign: 'top', w: el.position.w, x: el.position.x, y: el.position.y
      };
      ...
      const textOptions = {
        bold: el.style.bold, color: el.style.color, fontFace: el.style.fontFace, fontSize: el.style.fontSize, h: el.position.h, inset: 0, italic: el.style.italic, lineSpacing: el.style.lineSpacing, paraSpaceAfter: el.style.paraSpaceAfter, paraSpaceBefore: el.style.paraSpaceBefore, underline: el.style.underline, valign: 'top', w: adjustedW, x: adjustedX, y: el.position.y  // Remove default PowerPoint internal padding
      };
  ...
  return await page.evaluate(() => {
    ...
    const getPositionAndSize = (el, rect, rotation) => {
      ...
      if (isVertical) {
        ...
        return {
          h: rect.width, w: rect.height, x: centerX - rect.height / 2, y: centerY - rect.width / 2
        };
      }
      ...
      const centerY = rect.top + rect.height / 2;
      return {
        h: el.offsetHeight, w: el.offsetWidth, x: centerX - el.offsetWidth / 2, y: centerY - el.offsetHeight / 2
      };
    };
      ...
      element.childNodes.forEach((node) => {
        ...
        if (isText) {
          ...
          if (prevNodeIsText && prevRun) {
            ...
            runs.push({ options: { ...baseOptions }, text });
          }
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.className && el.className.includes('placeholder')) {
        ...
        if (rect.width === 0 || rect.height === 0) {
          ...
          placeholders.push({
            h: pxToInch(rect.height), id: el.id || `placeholder-${placeholders.length}`, w: pxToInch(rect.width), x: pxToInch(rect.left), y: pxToInch(rect.top)
          });
        }
        processed.add(el);
        return;
      ...
      if (el.tagName === 'IMG') {
        ...
        if (rect.width > 0 && rect.height > 0) {
          elements.push({
            position: {
              h: pxToInch(rect.height), w: pxToInch(rect.width), x: pxToInch(rect.left), y: pxToInch(rect.top)
            }, src: el.src, type: 'image'
          });
          processed.add(el);
          return;
        }
      ...
      if (isContainer) {
        ...
        if (hasBorder && !hasUniformBorder) {
          ...
          if (parseFloat(borderTop) > 0) {
            ...
            borderLines.push({
              color: rgbToHex(computed.borderTopColor), type: 'line', width: widthPt, x1: x, x2: x + w, y1: y + inset, y2: y + inset
            });
          }
          if (parseFloat(borderRight) > 0) {
            ...
            borderLines.push({
              color: rgbToHex(computed.borderRightColor), type: 'line', width: widthPt, x1: x + w - inset, x2: x + w - inset, y1: y, y2: y + h
            });
          }
          if (parseFloat(borderBottom) > 0) {
            ...
            const inset = (widthPt / 72) / 2;
            borderLines.push({
              color: rgbToHex(computed.borderBottomColor), type: 'line', width: widthPt, x1: x, x2: x + w, y1: y + h - inset, y2: y + h - inset
            });
          }
          if (parseFloat(borderLeft) > 0) {
            ...
            const inset = (widthPt / 72) / 2;
            borderLines.push({
              color: rgbToHex(computed.borderLeftColor), type: 'line', width: widthPt, x1: x + inset, x2: x + inset, y1: y, y2: y + h
            });
          }
        }
      ...
      if (hasFormatting) {
        ...
        elements.push({
          position: { h: pxToInch(h), w: pxToInch(w), x: pxToInch(x), y: pxToInch(y) }, style: adjustedStyle, text: runs, type: el.tagName.toLowerCase()
        });
      } else {
        ...
        elements.push({
          position: { h: pxToInch(h), w: pxToInch(w), x: pxToInch(x), y: pxToInch(y) }, style: {
            ...baseStyle,
            bold: isBold && !shouldSkipBold(computed.fontFamily),
            italic: computed.fontStyle === 'italic',
            underline: computed.textDecoration.includes('underline')
          }, text: transformedText, type: el.tagName.toLowerCase()
        });
      }
    ...
    return { background, elements, errors, placeholders };
  });
}
  ...
  try {
    ...
    try {
      ...
      await page.setViewportSize({
        height: Math.round(bodyDimensions.height), width: Math.round(bodyDimensions.width)
      });
    ...
    return { placeholders: slideData.placeholders, slide: targetSlide };
  } catch (error) {
