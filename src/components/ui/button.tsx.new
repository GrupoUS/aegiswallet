...
const UnifiedButton = React.forwardRef<HTMLButtonElement, UnifiedButtonProps>(
  ({
    ...
    const isNeumorph = variant?.includes('neumorph') || size?.includes('neumorph')
    const isGradient = variant?.includes('gradient') || variant === 'trust' || variant === 'success'
    ...
    // Apply gradient overlay for non-gradient buttons
      ...
      return (
        <div className="relative inline-flex">
          <div
            ...
            style={{
              background: `radial-gradient(${gradientSize}px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(172, 148, 105, 0.45), rgba(17, 32, 49, 0.15) 60%, transparent 80%)`,
            }}
            onMouseMove={(e) => {
