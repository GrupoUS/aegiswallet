...
export const INTENT_DEFINITIONS: Record<IntentType, IntentDefinition> = {
  ...
  [IntentType.CHECK_INCOME]: {
    ...
    patterns: [
      ...
      /\brecebimentos?\s+(do\s+)?m[êeê]s\b/i,
      /\bquanto\s+vai\s+entrar\b/i,
      /\bquando\s+cai\s+(o\s+)?sal[áa]rio\b/i,
    ],
    requiredSlots: [],
  ...
  [IntentType.FINANCIAL_PROJECTION]: {
    ...
    patterns: [
      ...
      /\b(como)\s+(vai|fica|ficar|esta|est[áÁ])\s+(ficar\s+)?(o\s+)?(meu\s+)?m[eê]s\??\b/i,
      ...
      /\bcomo\s+(vai|fica)\s+(ficar\s+)?(o\s+)?m[eê]s\??\b/i,
      /\bvai\s+sobrar\s+quanto\??$/i,
      /\bquanto\s+vai\s+faltar\??$/i,
      // Additional patterns for better coverage
      /^proje[çc][ãa]o\s+do\s+m[êeê]s\??$/i,
      /\bprevis[ãa]o\s+financeira\b/i,
      /\bbalan[çc]o\s+do\s+m[êeê]s\b/i,
      /\bestimativa\s+mensal\b/i,
      // Pattern for "como vai terminar o mês"
      /\bcomo\s+(vai|vou|vamos)\s+(terminar|acabar|fechar|encerrar)\s+(o\s+)?m[eêéèë]s\??\b/i,
    ],
