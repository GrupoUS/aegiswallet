...
async function processSyncQueue() {
	...
	console.log(
		`[${new Date().toISOString()}] Starting sync queue processing...`,
	);
			...
			try {
				...
				const action =
					item.sync_direction === 'to_google'
						...
						: 'sync_from_google';
				const bodyKey =
					item.sync_direction === 'to_google' ? 'event_id' : 'google_event_id';
				...
				if (newRetryCount >= MAX_RETRIES) {
					...
					console.error(
						`✗ Item ${item.id} permanently failed after ${MAX_RETRIES} retries`,
					);
					failed++;
