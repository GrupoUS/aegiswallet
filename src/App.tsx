...
const getOAuthErrorMessage = (
	...
	if (errorLower.includes('pkce') || errorLower.includes('code_verifier')) {
		return {
			description:
				'Sessão expirada. Por favor, limpe o cache do navegador e tente novamente.',
	...
	if (errorLower.includes('access_denied')) {
		return {
			description:
				'Você negou acesso à sua conta Google. Tente novamente se isso foi um erro.',
			title: 'Acesso negado',
		};
	}
	...
	if (errorLower.includes('redirect_uri_mismatch')) {
		return {
			description:
				'URL de redirecionamento inválida. Entre em contato com o suporte.',
