...
import { financialEvents, type FinancialEvent } from '@/db/schema/calendar';
 ...
 */
export async function processSyncQueue(limit: number = 10): Promise<void> {
	const now = new Date();
	...
	const pendingItems = await db.query.calendarSyncQueue.findMany({
		where: and(
			...
			or(
				isNull(calendarSyncQueue.scheduledFor),
				lt(calendarSyncQueue.scheduledFor, now),
			),
		),
	...
	for (const item of pendingItems) {
		try {
			...
			if (item.retryCount < item.maxRetries) {
				...
				const backoffMs = Math.pow(2, item.retryCount) * 1000 * 60; // 1, 2, 4 minutes
...
function convertGoogleToFinancialEvent(
	...
	return {
		...
		endDate,
		isAllDay,
		amount: amount ? (isIncome ? amount : -amount) : undefined,
