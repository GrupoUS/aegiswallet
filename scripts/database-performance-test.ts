...
export class BillingDatabasePerformanceTest {
  ...
  private async testSubscriptionQueryPerformance(): Promise<void> {
    ...
    try {
      ...
      console.log(`   ${passed ? '✅' : '❌'} ${passed ? 'PASSED' : 'FAILED'} - Target: <${PASS_THRESHOLD}ms`);
  ...
  private async testIndexUsageAnalysis(): Promise<void> {
    ...
    try {
      ...
      const unusedIndexes = indexUsage.rows.filter((row: any) => 
        row.idx_scan === 0 && criticalIndexes.includes(row.indexname)
      );

      if (unusedIndexes.length > 0) {
        issues.push(`Unused critical indexes: ${unusedIndexes.map((idx: any) => idx.indexname).join(', ')}`);
      }

      const totalScans = indexUsage.rows.reduce((sum: number, row: any) => sum + Number(row.idx_scan), 0);
      const totalReads = indexUsage.rows.reduce((sum: number, row: any) => sum + Number(row.idx_tup_read), 0);
      ...
      console.log(`   ✅ Critical Indexes: ${criticalIndexes.length - missingIndexes.length}/${criticalIndexes.length}`);
      console.log(`   ${missingIndexes.length === 0 ? '✅' : '❌'} ${missingIndexes.length === 0 ? 'PASSED' : 'FAILED'} - All critical indexes present`);
  ...
  private async testConcurrentOperations(): Promise<void> {
    ...
    try {
      ...
      if (failed > this.CONCURRENT_USERS * 0.1) { // Allow 10% failure rate
        issues.push(`High failure rate in concurrent operations: ${failed}/${this.CONCURRENT_USERS}`);
      }
      ...
      console.log(`   ${failed <= this.CONCURRENT_USERS * 0.1 ? '✅' : '❌'} ${failed <= this.CONCURRENT_USERS * 0.1 ? 'PASSED' : 'FAILED'} - Low failure rate`);
