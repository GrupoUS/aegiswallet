  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // Handle OAuth errors
        if (error) {
          secureLogger.error('OAuth callback error', { error, errorDescription });
          setStatus('error');
        ...
        if (code) {
          ...
          if (exchangeError) {
            secureLogger.error('PKCE exchange failed', { error: exchangeError.message });
            setStatus('error');
          ...
          if (data?.session) {
            secureLogger.authEvent('login_success', data.user?.id, { provider: 'oauth' });
            setStatus('success');
