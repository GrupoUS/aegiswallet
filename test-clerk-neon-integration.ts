...
if (!process.env.CLERK_SECRET_KEY) {
  console.warn('⚠️  CLERK_SECRET_KEY not found. Authentication will not work properly.');
}
  ...
  for (const tableName of tablesToCheck) {
    try {
      ...
      if (columns.length > 0) {
        ...
        console.log(`❌ ${tableName}: No user_id/clerk_user_id/organization_id column found!`);
      }
    } catch (error) {
      console.log(`❓ ${tableName}: Could not verify schema - ${error.message}`);
    }
  }
}
...
async function runIntegrationTests() {
  ...
  console.log('   https://clerk.com/docs/guides/development/integrations/databases/neon\n');
