...
export const VoiceComponentTemplate: React.FC<VoiceComponentProps> = ({
  ...
  const { 
    ...
    continuous: false, interimResults: true, language: 'pt-BR', onError: useCallback((error: string) => {
      console.error('Voice recognition error:', error);
      handleVoiceError(error);
    }, []), onResult: useCallback((transcript: string, confidence: number) => {
  ...
  const handleVoiceError = (error: string): void => {
    const errorMessages: Record<string, string> = {
      'aborted': 'Operação cancelada.', 'audio-capture': 'Problema com o microfone. Verifique se está conectado.', 'network': 'Problema de conexão. Verifique sua internet.', 'no-speech': 'Não ouvi nada. Poderia falar mais alto?', 'not-allowed': 'Permissão negada. Permita o acesso ao microfone.'
    };
  ...
  const processVoiceCommand = (transcript: string, confidence: number): VoiceCommand => {
    ...
    return {
      confidence, intent: detectedIntent, timestamp: new Date().toISOString(), transcript
    };
  };
