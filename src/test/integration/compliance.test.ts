...
describe.skipIf(!hasIntegrationTestEnv())('LGPD Compliance Integration Tests', () => {
	...
	describe('Consent Templates', () => {
		...
		it('should have mandatory consent templates', async () => {
			...
			const hasDataProcessing = templates.some((t) => t.consentType === 'data_processing');
	...
	describe('Consent Management', () => {
		beforeEach(async () => {
			...
			await db.delete(lgpdConsents).where(eq(lgpdConsents.userId, testUser.id));
		});
		...
		it('should prevent duplicate consents with same version', async () => {
			const [template] = await db
				...
				.from(consentTemplates)
				.where(
					and(eq(consentTemplates.consentType, 'analytics'), eq(consentTemplates.isActive, true)),
				)
				.limit(1);
			...
			const [updated] = await db
				...
				.onConflictDoUpdate({
					target: [lgpdConsents.userId, lgpdConsents.consentType, lgpdConsents.consentVersion],
					set: {
						consentTextHash: 'test-hash-updated',
						updatedAt: new Date(),
					},
	...
	describe('Data Export Requests', () => {
		beforeEach(async () => {
			await db.delete(lgpdExportRequests).where(eq(lgpdExportRequests.userId, testUser.id));
		});
	...
	describe('Data Deletion Requests', () => {
		beforeEach(async () => {
			await db.delete(dataDeletionRequests).where(eq(dataDeletionRequests.userId, testUser.id));
		});
	...
	describe('Transaction Limits', () => {
		beforeEach(async () => {
			await db.delete(transactionLimits).where(eq(transactionLimits.userId, testUser.id));
		});
	...
	describe('Audit Logging', () => {
		beforeEach(async () => {
			await db.delete(complianceAuditLogs).where(eq(complianceAuditLogs.userId, testUser.id));
		});
		...
		it('should create audit log entry', async () => {
			...
			expect((log.metadata as Record<string, unknown>)?.description).toBeDefined();
		});
	...
	describe('User Isolation', () => {
		it('should enforce user isolation for consents', async () => {
			...
			const [template] = await db
				...
				.from(consentTemplates)
				.where(
					and(eq(consentTemplates.consentType, 'marketing'), eq(consentTemplates.isActive, true)),
				)
				.limit(1);
