	...
	try {
		...
		try {
			...
			if (report.metrics.orphanedRecords === 0) {
				...
				report.issues.push(
					`Found ${orphanedPrefs.length} orphaned preferences and ${orphanedSec.length} orphaned security records`,
				);
				report.recommendations.push('Run cleanup-orphaned-data.ts script to fix orphaned records');
				console.log(
					`   ⚠️  Data integrity: ${report.metrics.orphanedRecords} orphaned records found`,
				);
			}
		} catch (error) {
		...
		try {
			...
			const [txCount, baCount, catCount] = await Promise.all([
				db
					.select()
					.from(transactions)
					.limit(1), // Just test table access
		...
		try {
			...
			if (auditLogCount.length > 0 && report.metrics.nullConstraints === 0) {
				...
				if (report.metrics.nullConstraints > 0) {
					report.issues.push(
						`Found ${report.metrics.nullConstraints} records with NULL user_id constraints`,
					);
				...
				if (auditLogCount.length === 0) {
					...
					report.recommendations.push(
						'Ensure audit logging is enabled for all user data operations',
					);
				}
		...
		const avgScore =
			(report.scores.connectivity +
				report.scores.integrity +
				report.scores.performance +
				report.scores.compliance) /
			4;
