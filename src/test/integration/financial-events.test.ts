	...
	() => {
		...
		const createEventRecord = async (event: Partial<FinancialEvent>) => {
			...
			const row = {
				...
				completed_at: sanitized.completedAt
					? sanitized.completedAt.toISOString()
					: null,
		...
		it('bloqueia criação quando faltam campos obrigatórios (validação)', () => {
			...
			expect(
				validation.errors.some((err: ValidationError) => err.field === 'title'),
			).toBe(true);
		});
		...
		it('reprova eventos com data final anterior à inicial', () => {
			...
			expect(validation.valid).toBe(false);
			expect(
				validation.errors.some((err: ValidationError) => err.field === 'end'),
			).toBe(true);
		});
