...
interface SmoothDrawerProps {
	...
	title?: string;
	/** Description for the drawer header */
	description?: string;
	/** Width preset for the drawer */
	width?: 'sm' | 'md' | 'lg' | 'xl' | 'full';
	/** Side of screen to slide from */
	side?: 'left' | 'right';
}

function SmoothDrawer({
	open,
	onOpenChange,
	children,
	title,
		...
		<SmoothDrawerContext.Provider value={{ open, setOpen: onOpenChange }}>
			<DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
				<AnimatePresence mode="wait">
					{open && (
						<SmoothDrawerContent title={title} description={description} width={width} side={side}>
							{children}
						</SmoothDrawerContent>
					)}
				</AnimatePresence>
...
interface DrawerStaggerContainerProps extends Omit<HTMLMotionProps<'div'>, 'ref'> {
	className?: string;
}

const DrawerStaggerContainer = React.forwardRef<HTMLDivElement, DrawerStaggerContainerProps>(
	({ className, ...props }, ref) => {
		return (
			<motion.div
				ref={ref}
...
interface DrawerStaggerItemProps extends Omit<HTMLMotionProps<'div'>, 'ref'> {
	className?: string;
}

const DrawerStaggerItem = React.forwardRef<HTMLDivElement, DrawerStaggerItemProps>(
	({ className, ...props }, ref) => {
