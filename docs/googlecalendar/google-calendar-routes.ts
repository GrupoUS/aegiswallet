...
googleCalendarRouter.get('/callback', async (c) => {
	...
	// Handle OAuth errors
	if (error) {
		secureLogger.warn('OAuth error from Google', { requestId, error });
	...
	try {
		...
		// Save tokens
		await saveTokens(userId, tokens, userInfo.data.email || undefined, userInfo.data.id || undefined);
