...
async function runTests() {
	...
	await runTest('PostgreSQL Version Check', async () => {
		...
		const rows =
			(result as { rows?: Array<{ pg_version: string }> }).rows || result;
	...
	await runTest('Drizzle schema type inference', async () => {
		...
		if (category) {
			...
			const _isSystem: boolean = category.isSystem;
			void _id;
			void _name;
			void _isSystem;
		}
	...
	await runTest('Bank account type validation', async () => {
		...
		if (account) {
			...
			const _balance: string | null = account.balance;
			const _currency: string | null = account.currency;
			const _isActive: boolean | null = account.isActive;
			void _id;
			void _balance;
			void _currency;
			void _isActive;
	...
	await runTest('Complex query latency (<500ms)', async () => {
		...
		if (latency > 500) {
			throw new Error(
				`Complex query latency too high: ${latency.toFixed(2)}ms`,
			);
		}
	});
	...
	await runTest('Concurrent queries (5 parallel)', async () => {
		...
		if (latency > 500) {
			throw new Error(
				`Parallel query latency too high: ${latency.toFixed(2)}ms`,
			);
		}
	});
	...
	await runTest(
		'Foreign key relationships (users → subscriptions)',
		async () => {
	...
	await runTest('Required fields not null (users.id)', async () => {
		const nullIds = await db.execute(sql`
			...
			WHERE id IS NULL
		`);
		const count = (nullIds as unknown as Array<{ null_count: number }>)[0]
			?.null_count;
		if (count > 0) {
			throw new Error(`Found ${count} users with null IDs`);
		}
	});
	...
	categories.forEach((cat) => {
		const catResults = results.filter((r) =>
			new RegExp(cat.pattern, 'i').test(r.name),
		);
		if (catResults.length > 0) {
			const avgDuration =
				catResults.reduce((sum, r) => sum + r.duration, 0) / catResults.length;
