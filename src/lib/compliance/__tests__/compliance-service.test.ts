...
describe('ComplianceService', () => {
  ...
  describe('grantConsent', () => {
    ...
    it('should throw error when template not found', async () => {
      ...
      ).rejects.toThrow('Modelo de consentimento não encontrado: data_processing');
    });
  });
  ...
  describe('checkRequiredConsents', () => {
    it('should return true when all required consents are granted', async () => {
      ...
      expect(mockSupabase._mockRpc).toHaveBeenCalledWith('check_required_consents', {
        p_user_id: 'user-123',
        ...
        if (table === 'lgpd_consents') {
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                eq: vi.fn().mockReturnValue({
                  is: vi.fn().mockResolvedValue({ data: mockUserConsents, error: null }),
                }),
              }),
            }),
          };
        ...
        if (table === 'data_export_requests') {
          return {
            insert: vi.fn().mockReturnValue({
              select: vi.fn().mockReturnValue({
                single: vi.fn().mockResolvedValue({ data: mockExportRequest, error: null }),
              }),
            }),
          };
        }
      ...
      mockSupabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'data_deletion_requests') {
          return {
            ...
            insert: vi.fn().mockReturnValue({
              select: vi.fn().mockReturnValue({
                single: vi.fn().mockResolvedValue({ data: mockDeletionRequest, error: null }),
              }),
            }),
          };
        }
  ...
  describe('checkTransactionLimit', () => {
    it('should allow transaction within limit', async () => {
      ...
      mockSupabase._mockFrom.mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                single: vi.fn().mockResolvedValue({ data: mockLimit, error: null }),
              }),
            }),
          }),
        }),
      ...
      const result = await service.checkTransactionLimit('user-123', 'pix_daytime', 1000);
    ...
    it('should allow transaction when no limit is configured', async () => {
      ...
      const result = await service.checkTransactionLimit('user-123', 'pix_daytime', 10000);
  ...
  describe('getAuditHistory', () => {
    it('should return audit logs for user', async () => {
      ...
      expect(mockSupabase._mockFrom).toHaveBeenCalledWith('compliance_audit_logs');
    ...
    it('should filter by event type when provided', async () => {
      ...
      await service.getAuditHistory('user-123', { eventType: 'consent_granted' });

      expect(mockEqEventType).toHaveBeenCalledWith('event_type', 'consent_granted');
    });
