...
async function renewExpiringChannels() {
	...
	console.log(
		`[${new Date().toISOString()}] Starting channel renewal check...`,
	);
	...
	try {
		...
		const { data: expiringChannels, error: queryError } = await supabase
			...
			.select(
				'user_id, google_channel_id, google_resource_id, channel_expiry_at',
			)
		...
		if (queryError) {
			throw new Error(
				`Failed to query expiring channels: ${queryError.message}`,
			);
		}
		...
		for (const channel of expiringChannels) {
			try {
				...
				console.error(
					`✗ Failed to renew channel for user ${channel.user_id}:`,
					error,
				);
				failed++;
				...
				if (retryCount >= maxRetries) {
					console.error(
						`✗ All retries failed for user ${channel.user_id}, disabling sync`,
					);
