...
function determineTargetDroid(command, complexity = 5) {
  ...
  if (
    autoRoutingRules.security_triggers.keywords.some((keyword) =>
  ...
  if (
    autoRoutingRules.research_triggers.keywords.some((keyword) =>
      lowerCommand.includes(keyword.toLowerCase())
    )
  ) {
    return {
  ...
  if (
    autoRoutingRules.database_triggers.keywords.some((keyword) =>
      lowerCommand.includes(keyword.toLowerCase())
    )
  ) {
    return {
  ...
  if (
    autoRoutingRules.testing_triggers.keywords.some((keyword) =>
      lowerCommand.includes(keyword.toLowerCase())
    )
  ) {
    return {
  ...
  if (
    autoRoutingRules.design_triggers.keywords.some((keyword) =>
      lowerCommand.includes(keyword.toLowerCase())
    )
  ) {
    return {
  ...
  if (
    autoRoutingRules.implementation_triggers.keywords.some((keyword) =>
      lowerCommand.includes(keyword.toLowerCase())
    )
  ) {
