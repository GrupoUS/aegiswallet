	...
	return {
		listContacts: tool({
            ...
            `)
					...
					let contacts = (data ?? []) as ContactWithPaymentMethods[];
					...
					const formattedContacts = contacts.map((contact) => ({
						...
						paymentMethods:
							contact.contact_payment_methods.map(filterSensitiveData),
		...
		addContact: tool({
			...
			execute: async ({ name, email, phone, cpf, isFavorite }) => {
				try {
					...
					const contact = data as ContactDbRow;
            ...
            `)
					...
					if (selectedMethod.payment_type === 'PIX') {
						...
						if (!selectedMethod.pix_key || !selectedMethod.pix_key_type) {
							throw new Error(
								'Método PIX selecionado não possui chave configurada',
							);
						}
