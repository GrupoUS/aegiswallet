...
const NOT_IMPLEMENTED_RESPONSE = {
	...
	message: 'Esta funcionalidade está temporariamente indisponível durante a migração do sistema.',
};
	...
	return c.json(
		{
			data: {
				...
				isEnabled: false,
				lastSyncAt: null,
				migrationNotice: 'Google Calendar sync is being migrated to new infrastructure',
			},
			meta: {
				requestId,
 ...
 */
googleCalendarRouter.put('/sync/settings', authMiddleware, rateLimitMiddleware, async (c) => {
	const requestId = c.get('requestId');
 ...
 */
googleCalendarRouter.post('/sync/event', authMiddleware, rateLimitMiddleware, async (c) => {
	const requestId = c.get('requestId');
