...
async function validateConstraints() {
	...
	try {
		...
		const orphanedPreferences = await db.select().from(userPreferences).where(
		...
		console.log(
			`   ✅ User Preferences: ${allPreferences.length} total, ${orphanedPrefs.length} orphaned`,
		);
		...
		console.log(
			`   ✅ Transactions: ${allTransactions.length} total, ${orphanedTx.length} orphaned`,
		);
		...
		console.log(
			`   ✅ Bank Accounts: ${allBankAccounts.length} total, ${orphanedBa.length} orphaned`,
		);
		...
		console.log(
			`   ✅ Transaction Categories: ${allCategories.length} total, ${orphanedCat.length} orphaned`,
		);
