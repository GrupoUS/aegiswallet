...
const createMockSupabaseClient = (jwtPayload?: any) => {
  const client = createClient<Database>(
    ...
    'test-anon-key', // Test key
    {
      ...
      global: {
        headers: {
          Authorization: jwtPayload ? `Bearer ${createMockJWT(jwtPayload)}` : undefined,
        },
      },
    }
  );
...
describe('Supabase RLS Policy Testing', () => {
  ...
  describe('Patient Table RLS', () => {
    describe('Anonymous Users', () => {
      it('should deny access to patient data without authentication', async () => {
        await expect(testRLSPolicy('anon', {}, 'select', 'patients', false)).resolves.not.toThrow();
      });

      it('should deny patient creation without authentication', async () => {
        await expect(testRLSPolicy('anon', {}, 'insert', 'patients', false)).resolves.not.toThrow();
      });

      it('should deny patient updates without authentication', async () => {
        await expect(testRLSPolicy('anon', {}, 'update', 'patients', false)).resolves.not.toThrow();
      });

      it('should deny patient deletion without authentication', async () => {
        await expect(testRLSPolicy('anon', {}, 'delete', 'patients', false)).resolves.not.toThrow();
      });
    });
    ...
    describe('Authenticated Patients', () => {
      ...
      it('should allow patients to read their own data', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'select', 'patients', true)
        ).resolves.not.toThrow();
      });

      it('should allow patients to create their own records', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'insert', 'patients', true)
        ).resolves.not.toThrow();
      });

      it('should allow patients to update their own data', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'update', 'patients', true)
        ).resolves.not.toThrow();
      });
    ...
    describe('Healthcare Professionals', () => {
      ...
      it('should allow doctors to read patient data for their department', async () => {
        await expect(
          testRLSPolicy('authenticated', doctorContext, 'select', 'patients', true)
        ).resolves.not.toThrow();
      });

      it('should allow doctors to update patient medical records', async () => {
        await expect(
          testRLSPolicy('authenticated', doctorContext, 'update', 'patients', true)
        ).resolves.not.toThrow();
      });

      it('should deny doctors from deleting patient data', async () => {
        await expect(
          testRLSPolicy('authenticated', doctorContext, 'delete', 'patients', false)
        ).resolves.not.toThrow();
      });
    });
    ...
    describe('Administrators', () => {
      ...
      it('should allow administrators full access to patient data', async () => {
        await expect(
          testRLSPolicy('authenticated', adminContext, 'select', 'patients', true)
        ).resolves.not.toThrow();

        await expect(
          testRLSPolicy('authenticated', adminContext, 'update', 'patients', true)
        ).resolves.not.toThrow();

        await expect(
          testRLSPolicy('authenticated', adminContext, 'delete', 'patients', true)
        ).resolves.not.toThrow();
      });
    });
  });
  ...
  describe('Appointments Table RLS', () => {
    describe('Patient Access', () => {
      ...
      it('should allow patients to read their own appointments', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'select', 'appointments', true)
        ).resolves.not.toThrow();
      });

      it('should allow patients to create appointment requests', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'insert', 'appointments', true)
        ).resolves.not.toThrow();
      });

      it('should deny patients from modifying appointment details', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'update', 'appointments', false)
        ).resolves.not.toThrow();
      });
    });
    ...
    describe('Doctor Access', () => {
      ...
      it('should allow doctors to read their assigned appointments', async () => {
        await expect(
          testRLSPolicy('authenticated', doctorContext, 'select', 'appointments', true)
        ).resolves.not.toThrow();
      });

      it('should allow doctors to update appointment status', async () => {
        await expect(
          testRLSPolicy('authenticated', doctorContext, 'update', 'appointments', true)
        ).resolves.not.toThrow();
      });
    });
  });
  ...
  describe('Payments Table RLS', () => {
    describe('Patient Access', () => {
      ...
      it('should allow patients to read their own payment history', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'select', 'payments', true)
        ).resolves.not.toThrow();
      });

      it('should deny patients from modifying payment records', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'update', 'payments', false)
        ).resolves.not.toThrow();
      });

      it('should deny patients from deleting payment records', async () => {
        await expect(
          testRLSPolicy('authenticated', patientContext, 'delete', 'payments', false)
        ).resolves.not.toThrow();
      });
    });
    ...
    describe('Billing Department Access', () => {
      ...
      it('should allow billing staff to read all payment records', async () => {
        await expect(
          testRLSPolicy('authenticated', billingContext, 'select', 'payments', true)
        ).resolves.not.toThrow();
      });

      it('should allow billing staff to process payments', async () => {
        await expect(
          testRLSPolicy('authenticated', billingContext, 'insert', 'payments', true)
        ).resolves.not.toThrow();
      });
    });
  });
  ...
  describe('Audit Trail RLS', () => {
    ...
    describe('System-Level Access', () => {
      ...
      it('should allow system processes to write audit logs', async () => {
        await expect(
          testRLSPolicy('authenticated', systemContext, 'insert', 'audit_logs', true)
        ).resolves.not.toThrow();
      });
    });
  });
  ...
  describe('JWT Token Validation', () => {
    ...
    it('should accept valid JWT tokens with proper claims', () => {
      ...
      expect(validJWT).toMatch(/^[A-Za-z0-9-_=+/]+\.[A-Za-z0-9-_=+/]+\.[A-Za-z0-9-_=+/]+$/);
    });
  });
});
