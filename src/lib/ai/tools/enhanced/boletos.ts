    ...
    listBoletos: tool({
      ...
      execute: async ({
        ...
        try {
          ...
          if (error) {
            secureLogger.error('Erro ao listar boletos', { error: error.message, userId });
      ...
      execute: async ({ boletoId, accountId, paymentDate, confirmPayment }) => {
        try {
          ...
          if (updateError) {
            ...
            throw new Error(`Erro ao processar pagamento: ${updateError.message}`);
          }
      ...
      execute: async ({ boletoId, scheduledDate, accountId, reminderDays }) => {
        try {
          ...
          if (scheduleError) {
            ...
            throw new Error(`Erro ao agendar pagamento: ${scheduleError.message}`);
          }
