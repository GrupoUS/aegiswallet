...
CREATE TABLE IF NOT EXISTS public.calendar_sync_mapping (
    ...
    sync_status TEXT CHECK (sync_status IN ('synced', 'pending', 'error', 'conflict')),
    sync_direction TEXT CHECK (sync_direction IN ('aegis_to_google', 'google_to_aegis', 'bidirectional')),
    error_message TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.calendar_sync_audit (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    action TEXT CHECK (action IN ('sync_started', 'sync_completed', 'sync_failed', 'event_created', 'event_updated', 'event_deleted')),
    event_id UUID,
    details JSONB,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Create policies
CREATE POLICY "Users can manage their own tokens" ON public.google_calendar_tokens
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can manage their own sync mappings" ON public.calendar_sync_mapping
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can manage their own sync settings" ON public.calendar_sync_settings
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own audit logs" ON public.calendar_sync_audit
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own audit logs" ON public.calendar_sync_audit
...
CREATE INDEX IF NOT EXISTS idx_calendar_sync_mapping_aegis_id ON public.calendar_sync_mapping(aegis_event_id);
CREATE INDEX IF NOT EXISTS idx_calendar_sync_mapping_google_id ON public.calendar_sync_mapping(google_event_id);
CREATE INDEX IF NOT EXISTS idx_calendar_sync_audit_user_created ON public.calendar_sync_audit(user_id, created_at);
...
CREATE TRIGGER update_calendar_sync_mapping_updated_at
