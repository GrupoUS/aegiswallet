...
async function promptForValue(question) {
  ...
  if (supabaseResult.status !== 0) {
    output.write(
      '\n⚠️  Não foi possível aplicar os segredos automaticamente. Execute manualmente:\n' +
        `supabase secrets set GOOGLE_CLIENT_ID=${answers.GOOGLE_OAUTH_CLIENT_ID} GOOGLE_CLIENT_SECRET=${answers.GOOGLE_OAUTH_CLIENT_SECRET} GOOGLE_REDIRECT_URI=${answers.GOOGLE_OAUTH_REDIRECT_URI}${projectRef ? ` --project-ref ${projectRef}` : ''}\n`
    );
  }
