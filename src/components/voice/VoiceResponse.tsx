...
import { CheckCircle, AlertCircle, Info, TrendingUp, CreditCard, ArrowUpRight } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { cn } from '@/lib/utils';

interface VoiceResponseProps {
  type: 'success' | 'error' | 'info' | 'balance' | 'budget' | 'bills' | 'incoming' | 'projection' | 'transfer';
  message: string;
  data?: any;
  className?: string;
}
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className={cn(
                  ...
                  data.spentPercentage > 90 ? "bg-red-500" : 
                  data.spentPercentage > 70 ? "bg-orange-500" : "bg-green-500"
                )}
            ...
            {data.variation && (
              <div className="flex justify-between items-center">
                ...
                <span className={cn(
                  ...
                  {data.variation >= 0 ? '+' : ''}{formatCurrency(data.variation)}
                </span>
              </div>
            )}
          </div>
