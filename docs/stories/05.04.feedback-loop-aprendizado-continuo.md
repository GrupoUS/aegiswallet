...
- [ ] **Sistema de Captura de Feedback Implícito** (AC: 1)
  - [ ] Implementar `src/lib/ml/feedbackCollector.ts` para tracking automático
  - [ ] Configurar eventos de aceitação: usuário não contesta decisão em 24h
  - [ ] Adicionar tracking de reversões: usuário desfaz ação da IA
  - [ ] Implementar coleta de tempo de resposta: aceita rápido vs demora
  - [ ] Configurar tracking de padrões: usuário repete ação sugerida
  - [ ] Adicionar métricas de engajamento: tempo gasto visualizando explicações
  - [ ] Implementar coleta de contexto: horário, dispositivo, localização
  - [ ] Configurar batch processing para envio eficiente ao backend

- [ ] **Interface de Feedback Explícito** (AC: 2)
  - [ ] Criar `src/components/feedback/FeedbackWidget.tsx` contextual
  - [ ] Implementar ratings de 1-5 estrelas para cada decisão
  - [ ] Adicionar feedback binário: 👍👎 para ações rápidas
  - [ ] Configurar feedback textual opcional com sugestões
  - [ ] Implementar feedback por categoria: "Muito conservador", "Muito agressivo"
  - [ ] Adicionar feedback de explicação: "Entendi", "Confuso", "Incorreto"
  - [ ] Configurar prompts inteligentes: aparecem após 3 decisões similares
  - [ ] Implementar gamificação: pontos por feedback útil

- [ ] **Pipeline de Retreinamento Automatizado** (AC: 3, 7)
  - [ ] Implementar `src/lib/ml/retrainingPipeline.ts` com agenda semanal
  - [ ] Configurar coleta de dados: feedback + decisões + outcomes
  - [ ] Adicionar pré-processamento: limpeza, normalização, feature engineering
  - [ ] Implementar validação cruzada: 80% treino, 20% validação
  - [ ] Configurar métricas de avaliação: precisão, recall, F1-score
  - [ ] Adicionar comparação com modelo anterior: A/B testing automático
  - [ ] Implementar deploy condicional: só atualiza se melhoria >2%
  - [ ] Configurar rollback automático se performance degradar

- [ ] **Sistema de Personalização Adaptativa** (AC: 4, 6)
  - [ ] Implementar `src/lib/ml/personalizer.ts` com perfis individuais
  - [ ] Configurar clustering de usuários: conservador, moderado, agressivo
  - [ ] Adicionar pesos adaptativos por categoria de decisão
  - [ ] Implementar transfer learning: novos usuários herdam de cluster
  - [ ] Configurar drift detection: mudanças no comportamento do usuário
  - [ ] Adicionar re-clustering mensal baseado em feedback recente
  - [ ] Implementar cold start: primeiras 10 decisões com modelo geral
  - [ ] Configurar métricas de personalização: melhoria vs baseline
...
- [ ] **Infraestrutura de ML** (AC: 3, 7)
  ...
  - [ ] Configurar monitoramento de performance: latência, throughput
  - [ ] Implementar cache de predições: TTL baseado em volatilidade
  - [ ] Adicionar health checks para pipeline de ML
  - [ ] Configurar alertas para falhas de retreinamento
  - [ ] Implementar backup automático de modelos críticos

- [ ] **Privacidade e Segurança** (AC: 8)
  - [ ] Implementar differential privacy para dados de treinamento
  - [ ] Configurar anonimização: hash de IDs, remoção de PII
  - [ ] Adicionar federated learning: modelos locais + agregação
  - [ ] Implementar auditoria de modelos: bias detection, fairness metrics
  - [ ] Configurar retenção de dados: 12 meses para feedback, 24 para modelos
  - [ ] Adicionar consentimento granular: opt-out por tipo de aprendizado
  - [ ] Implementar right to be forgotten: remoção de dados específicos
  - [ ] Configurar compliance LGPD: relatórios de uso de dados
...
- `src/lib/ml/personalizer.ts` - Personalização por usuário
- `src/lib/ml/modelManager.ts` - Gerenciamento de modelos
- `supabase/functions/ml-training/` - Edge Functions para ML
- `src/lib/ml/privacyEngine.ts` - Privacidade e anonimização
...
class RetrainingPipeline {
  ...
  async collectTrainingData(timeRange: string): Promise<TrainingData> {
    // 1. Buscar feedback dos últimos 7 dias
    ...
    // 2. Converter para features numéricas
  ...
  private extractLabel(feedback: any): number {
    // Converter feedback para label binário
...
CREATE TABLE user_feedback (
  ...
  response_time_ms INTEGER, -- tempo para reagir à decisão

  -- Contexto
  context_data JSONB, -- dispositivo, localização, horário
...
CREATE TABLE ml_models (
  ...
  model_metadata JSONB, -- arquitetura, hiperparâmetros
...
CREATE TABLE user_personalization (
  ...
  personalization_score DECIMAL(5,4), -- quão personalizado está
...
CREATE TABLE ml_training_jobs (
  ...
  -- Configuração
  config JSONB, -- parâmetros do treinamento
