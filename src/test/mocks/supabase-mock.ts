...
class MockDatabase {
	...
	find<T extends TableName>(
		table: T,
		predicate: (record: Row<T>) => boolean,
	): Row<T>[] {
		const tableData = this.data.get(table) || [];
...
class MockQueryBuilder<T extends TableName> {
	...
	contains<K extends keyof Row<T>>(
		_column: K,
		_value: unknown,
	): MockQueryBuilder<T> {
		return this; // Mock implementation
	...
	insert(
		record: Insert<T>,
	): MockQueryBuilder<T> & { data: Row<T> | null; error: any } {
	...
	update(
		_updates: Update<T>,
	): MockQueryBuilder<T> & { data: Row<T>[] | null; error: unknown } {
		return this as unknown as MockQueryBuilder<T> & {
			data: Row<T>[] | null;
			error: unknown;
		};
	}
...
export const setupMockDatabase = {
	...
	insert: <T extends TableName>(table: T, record: Insert<T>) =>
