...
describe.skipIf(!hasIntegrationTestEnv())('Bank Accounts API Integration', () => {
	...
	const createAccountDirect = async (accountData: Partial<InsertBankAccount>) => {
		const [data] = await db
			.insert(bankAccounts)
			.values({
				userId: testUser.id,
				...
				accountMask: accountData.accountMask || `**** ${Math.floor(1000 + Math.random() * 9000)}`,
	...
	const getAccountById = async (id: string) => {
		const [data] = await db
			.select()
			.from(bankAccounts)
			.where(and(eq(bankAccounts.id, id), eq(bankAccounts.userId, testUser.id)))
			.limit(1);
		return data ?? null;
	};
	...
	it('updates account information correctly', async () => {
		...
		const [updated] = await db
			...
			.where(and(eq(bankAccounts.id, created.id), eq(bankAccounts.userId, testUser.id)))
			.returning();
	...
	it('removes account and verifies deletion', async () => {
		...
		await db
			.delete(bankAccounts)
			.where(and(eq(bankAccounts.id, accountId), eq(bankAccounts.userId, testUser.id)));
	...
	it('calculates total balance aggregated by currency', async () => {
		...
		const accounts = await db
			...
			.from(bankAccounts)
			.where(and(eq(bankAccounts.userId, testUser.id), eq(bankAccounts.isActive, true)));
