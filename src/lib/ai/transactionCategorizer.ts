...
const BRAZILIAN_CATEGORIES: Category[] = [
  {
    ...
    icon: '🍽️',
    color: '#ef4444',
    keywords: ['restaurante', 'lanchonete', 'supermercado', 'mercado', 'padaria', 'delivery', 'ifood', 'rappi', 'food', 'comida', 'alimentação'],
    patterns: [
      /mercado.*super/i,
      /restaurante/i,
      /lanchonete/i,
    ...
    subcategories: [
      {
        id: 'groceries',
        name: 'Supermercado',
        keywords: ['supermercado', 'mercado', 'atacarejo', 'atacadão', 'carrefour', 'pão de açúcar'],
  ...
  },
  {
    ...
    icon: '🚗',
    color: '#3b82f6',
    keywords: ['uber', '99', 'taxi', 'posto', 'gasolina', 'estacionamento', 'transporte', 'ônibus', 'metrô'],
    patterns: [
      /uber/i,
      /99/i,
      /taxi/i,
...
export class TransactionCategorizer {
   ...
   */
  categorizeTransaction(transaction: Transaction): Transaction & { category: string; subcategory?: string; confidence: number } {
   ...
   */
  categorizeTransactions(transactions: Transaction[]): Array<Transaction & { category: string; subcategory?: string; confidence: number }> {
   ...
   */
  learnFromCorrection(transaction: Transaction, correctCategory: string, correctSubcategory?: string): void {
   ...
   */
  getCategoryInsights(transactions: Array<Transaction & { category: string; subcategory?: string }>) {
  ...
  predictSpending(transactions: Array<Transaction & { category: string }>, daysAhead: number = 30) {
    ...
    const recentTransactions = transactions.filter(t => 
      t.date >= oneMonthAgo && t.type === 'expense'
    );
    ...
    const predictions = this.getCategories()
      ...
      .map(category => {
        ...
        return {
          ...
          confidence: categoryTransactions.length > 0 ? Math.min(categoryTransactions.length / 10, 1) : 0.1
        };
      })
