	...
	return {
		...
		addContactPaymentMethod: tool({
			...
			execute: async ({
				...
				try {
					const contact = await validateContactExists(
						supabase,
						userId,
						contactId,
					);
		...
		sendToContact: tool({
			...
			execute: async ({
				...
				try {
					const { contact, paymentMethods } =
						await fetchContactWithPaymentMethods(supabase, userId, contactId);
					const selectedMethod = selectPaymentMethod(
						paymentMethods,
						paymentMethodId,
						paymentType,
					);
					...
					return buildSendToContactResponse(
						transferResult,
						contact,
						selectedMethod,
						amount,
				...
				try {
					if (!confirmDeletion) {
						...
						return {
							...
							contactName: contact.name,
							paymentMethodsCount:
								contact.contact_payment_methods?.[0]?.count || 0,
