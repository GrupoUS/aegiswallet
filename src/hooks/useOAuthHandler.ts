...
const validatePKCEState = (): {
	...
	try {
		...
		diagnostics.hasAuthToken = storageKeys.some((key) =>
			key.includes('auth-token'),
		);
	} catch (e) {
		diagnostics.storageAccessError =
		...
		const processOAuthQuery = async () => {
			...
			try {
				...
				if (exchangeError) {
					...
					secureLogger.error(
						'Failed to exchange PKCE code for session',
						errorContext,
					);
					...
					if (exchangeError.message?.toLowerCase().includes('invalid')) {
						userMessage =
							'Invalid API key. Verifique a configuração do Supabase.';
					} else if (exchangeError.message?.toLowerCase().includes('expired')) {
						userMessage =
							'Código de autenticação expirado. Tente fazer login novamente.';
					}
