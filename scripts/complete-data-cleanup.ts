	...
	try {
		...
		for (const user of users) {
			...
			try {
				...
				console.log(
					`   ❌ Failed to delete ${user.id}: ${error instanceof Error ? error.message : error}`,
				);
			}
		}
		...
		await runAsServiceAccount(async (tx) => {
			...
			for (const table of keyTables) {
				try {
					...
					console.log(
						`   ${table}: ${remaining === 0 ? '✅ Empty' : `❌ ${remaining} rows remaining`}`,
					);
				} catch {
...
async function verifyCleanState() {
	...
	try {
		const { data: users } = await clerkClient.users.getUserList({ limit: 100 });
		console.log(
			`   Clerk users: ${users.length === 0 ? '✅ 0 (Clean)' : `❌ ${users.length} remaining`}`,
		);
	} catch (error) {
	...
	try {
		...
		console.log(
			`   Neon users: ${Number(usersCount.rows[0]?.count) === 0 ? '✅ 0 (Clean)' : `❌ ${usersCount.rows[0]?.count} remaining`}`,
		);
		console.log(
			`   Neon bank_accounts: ${Number(bankCount.rows[0]?.count) === 0 ? '✅ 0 (Clean)' : `❌ ${bankCount.rows[0]?.count} remaining`}`,
		);
		console.log(
			`   Neon organizations: ${Number(orgCount.rows[0]?.count) === 0 ? '✅ 0 (Clean)' : `❌ ${orgCount.rows[0]?.count} remaining`}`,
		);
	} catch (error) {
