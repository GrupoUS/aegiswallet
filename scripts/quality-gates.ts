...
interface QualityGatesReport {
  ...
  brazilianCompliance: {
    lgpdStatus: string;
    portugueseValidation: string;
    pixCompliance: string;
...
const QUALITY_GATES = [
  ...
  },
  {
    name: 'Compliance',
    tool: 'LGPD',
    command: 'bunx oxlint --plugin=jsx-a11y --quiet src scripts && echo "LGPD validation completed"',
    threshold: '100% Compliance',
    weight: 1
  }
];

function executeGate(gate: typeof QUALITY_GATES[0]): QualityGateResult {
...
function parseMetrics(output: string, gateName: string): Record<string, number> {
  ...
  switch (gateName) {
    ...
    case 'Testing': {
      ...
      if (testMatch) metrics.passed = parseInt(testMatch[1]);
    ...
    case 'Security': {
      ...
      if (securityMatch) metrics.warnings = parseInt(securityMatch[1]);
...
function generateBrazilianComplianceStatus(gates: QualityGateResult[]) {
...
function generateReport(results: QualityGateResult[]): QualityGatesReport {
...
function printReport(report: QualityGatesReport) {
  console.log(`\n${'='.repeat(80)}`);
