...
function calculateQualityMetrics(results, biomeScore) {
...
function generateTestReport(results, biomeScore) {
  ...
  try {
    ...
    for (const suite of HEALTHCARE_TEST_SUITES) {
      try {
        ...
        results.push(result);
        const _status = result.status === 'passed' ? '✅' : result.status === 'failed' ? '❌' : '⏭️';
