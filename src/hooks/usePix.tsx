...
export function usePixFavorites() {
  const {
    ...
    isLoading,
    error,
  } = trpc.pix.getFavorites.useQuery();
...
export function usePixTransactions(filters?: {
  ...
  const { mutate: createTransaction } = trpc.pix.createTransaction.useMutation({
    ...
    onError: (mutationError) => {
      toast.error(
        mutationError.message || "Erro ao realizar transferência PIX",
      );
    },
  });
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            ...
            toast.success(
              `PIX recebido: R$ ${(transaction.amount ?? 0).toFixed(2)}`,
              {
                description:
...
export function usePixTransaction(id: string) {
  const {
    ...
    isLoading,
    error,
  } = trpc.pix.getTransaction.useQuery({ id });
...
export function usePixStats(period: "24h" | "7d" | "30d" | "1y" = "30d") {
  const {
    ...
    isLoading,
    error,
  } = trpc.pix.getStats.useQuery({ period });
...
export function usePixQRCodes() {
  ...
  const { mutate: generateQRCode, isPending: isGenerating } =
    trpc.pix.generateQRCode.useMutation({
      onSuccess: () => {
        ...
        (payload) => {
          ...
          if (updated.status === "completed") {
            ...
            toast.error("Transação PIX falhou", {
              description:
                updated.errorMessage ?? updated.error_message ?? undefined,
            });
          }
