...
- `src/components/ai/DecisionCard.tsx` - Card individual de decisão
- `src/components/ai/OpportunityPanel.tsx` - Painel de oportunidades
- `src/components/ai/FeedbackSystem.tsx` - Sistema de feedback
- `src/components/ai/DecisionTimeline.tsx` - Timeline de decisões
- `src/components/ai/ExplanationModal.tsx` - Modal de explicação detalhada

**Hooks Necessários:**
- `src/hooks/useDecisionHistory.ts` - Histórico de decisões
- `src/hooks/useExplanations.ts` - Explicações detalhadas
- `src/hooks/useDecisionFeedback.ts` - Sistema de feedback
- `src/hooks/useOpportunities.ts` - Oportunidades futuras

### Sistema de Explicações Detalhado
...
const explanationTemplates: ExplanationTemplate[] = [
  {
    ...
    examples: [
      "Paguei sua conta de energia no valor de R$ 150,00 porque você sempre paga no dia 4 e tem saldo suficiente.",
      "Paguei sua conta de internet no valor de R$ 89,90 porque é um pagamento recorrente e está dentro do orçamento."
    ]
  },
  {
    type: 'alert',
    template: "Alertei sobre {topic} porque {opportunity} baseado no seu padrão de {period}.",
    variables: ['topic', 'opportunity', 'period'],
    examples: [
      "Alertei sobre mudança de plano porque pode economizar R$ 45,00 baseado no seu padrão dos últimos 3 meses.",
      "Alertei sobre investimento porque você tem R$ 500,00 parados há mais de 30 dias."
    ]
  },
  {
    type: 'block',
    template: "Bloqueei {action} porque {reason} e preciso de sua confirmação.",
    variables: ['action', 'reason'],
    examples: [
      "Bloqueei a transferência de R$ 2.000,00 porque excede seu limite diário de R$ 1.500,00.",
      "Bloqueei o pagamento porque não reconheço este beneficiário em seu histórico."
    ]
  }
];
  ...
  private generateReasoning(decision: AIDecision): string[] {
    ...
    if (decision.type === 'payment') {
      if (decision.isRecurring) {
        reasons.push(`É um pagamento recorrente que você faz há ${decision.recurringMonths} meses`);
      }
      if (decision.balanceCheck) {
        reasons.push(`Seu saldo atual (${formatCurrency(decision.balance)}) é suficiente`);
      }
      if (decision.withinBudget) {
        reasons.push(`Está dentro do orçamento de ${decision.category} (${decision.budgetUsage}% usado)`);
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Transparência da IA</h2>
...
export function FeedbackSystem({ decisionId, onFeedbackSubmitted }: FeedbackSystemProps) {
  ...
  return (
    <div className="border rounded-lg p-4 space-y-4">
      <h4 className="font-medium">Esta explicação foi útil?</h4>
