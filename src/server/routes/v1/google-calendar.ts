	...
	async (c) => {
		...
		try {
			...
			if (tokensError && tokensError.code !== 'PGRST116') {
				throw new Error(
					`Erro ao verificar status de conexão: ${tokensError.message}`,
				);
			}
			...
			if (settingsError && settingsError.code !== 'PGRST116') {
				throw new Error(
					`Erro ao buscar configurações: ${settingsError.message}`,
				);
			}
	...
	async (c) => {
		...
		try {
			...
			const settings: CalendarSyncSettings = {
				...
				sync_direction:
					(data.sync_direction as CalendarSyncSettings['sync_direction']) ??
					'one_way_to_google',
	...
	async (c) => {
		...
		try {
			...
			if (input.auto_sync_interval_minutes !== undefined) {
				updateData.auto_sync_interval_minutes =
...
googleCalendarRouter.post(
	...
	async (c) => {
		...
		try {
			...
			const action =
				input.direction === 'to_google' ? 'sync_to_google' : 'sync_from_google';
			const bodyKey =
				input.direction === 'to_google' ? 'event_id' : 'google_event_id';
