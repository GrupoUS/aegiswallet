...
const ProcessingRecordSchema = z.object({
  ...
  purpose: ConsentPurposes,
  legalBasis: z.enum(['CONSENT', 'LEGAL_OBLIGATION', 'CONTRACT', 'VITAL_INTEREST', 'PUBLIC_INTEREST', 'LEGITIMATE_INTEREST'])
})
...
lgpdRouter.get('/consent', async (c) => {
  ...
  try {
    ...
    const latestConsents = consents?.reduce((acc, consent) => {
      if (!acc[consent.purpose] || new Date(consent.timestamp) > new Date(acc[consent.purpose].timestamp)) {
  ...
  if (masked.transactions) {
    masked.transactions = masked.transactions.map((tx: any) => ({
      ...
      recipient_name: tx.recipient_name ? 
        tx.recipient_name.substring(0, 2) + '***' : null
    }))
  }
  ...
  if (masked.profile) {
    masked.profile = {
      ...
      cpf: masked.profile.cpf ? 
        masked.profile.cpf.replace(/(\d{3})\d{6}(\d{2})/, '$1******$2') : null,
      email: masked.profile.email ? 
        masked.profile.email.replace(/(.{2}).*@/, '$1***@') : null
    }
  }
