...
export interface SecurityEvent {
  userId: string
  eventType: 'login_attempt' | 'auth_success' | 'auth_failure' | 'account_locked' | 'suspicious_activity'
  timestamp: Date
  ipAddress: string
  userAgent: string
...
export class FraudDetectionService {
  ...
  async analyzeSecurityEvent(event: SecurityEvent): Promise<FraudDetectionResult> {
    ...
    try {
      ...
      console.error('[FraudDetection] Analysis failed:', error)
      return {
        riskScore: 0,
        riskLevel: 'low',
        detectedAnomalies: [],
  ...
  private async analyzeLocationPatterns(
    ...
    for (const recentLocation of recentLocations) {
      ...
      const timeDiff = (event.timestamp.getTime() - recentLocation.timestamp.getTime()) / (1000 * 60 * 60) // hours
      const speed = distance / timeDiff

      if (speed > this.config.patterns.get('impossible_travel')?.threshold || 1000) {
        score += 0.8
    ...
    if (locationCount) {
      const uniqueLocations = new Set(
        locationCount
          .map(event => event.metadata?.location?.city)
          .filter(Boolean)
      ).size
  ...
  private async analyzeDevicePatterns(
    ...
    const isNewLocation = !event.location || !userProfile.knownLocations.some(
      loc => loc.country === event.location!.country && loc.city === event.location!.city
    )
    ...
    if (deviceCount) {
      const uniqueDevices = new Set(
        deviceCount
          .map(event => event.device_fingerprint)
          .filter(Boolean)
      ).size
   ...
   */
  private async getRecentUserLocations(userId: string): Promise<Array<{
    latitude: number
    longitude: number
    timestamp: Date
  }>> {
  ...
  private async getRecentAuthMethods(userId: string): Promise<string[]> {
    ...
    return data
      .map(event => event.metadata?.authMethod)
  ...
  private async analyzeAccountTakeoverPatterns(event: SecurityEvent): Promise<number> {
    ...
    if (recentEvents && recentEvents.length >= 3) {
      const failuresBeforeSuccess = recentEvents.slice(0, -1).filter(e => e.event_type === 'auth_failure')
      ...
      if (failuresBeforeSuccess.length >= 2 && lastEvent.event_type === 'auth_success') {
        ...
        if (failureLocation && successLocation && 
  ...
  private async logFraudDetection(
    ...
    try {
      ...
      console.error('[FraudDetection] Failed to log detection result:', error)
    }
  }
