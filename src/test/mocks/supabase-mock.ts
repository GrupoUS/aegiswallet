...
class SimpleMockDatabase {
	...
	findOne(table: string, predicate: (record: unknown) => boolean): unknown | null {
...
class SimpleMockQueryBuilder {
	...
	eq(column: string, value: unknown): SimpleMockQueryBuilder {
		return this;
	}
...
export const setupMockDatabase = {
	...
	insert: (table: string, record: unknown) => mockDatabase.insert(table, record),
	find: (table: string, predicate: (record: unknown) => boolean) => mockDatabase.find(table, predicate),
	findOne: (table: string, predicate: (record: unknown) => boolean) => mockDatabase.findOne(table, predicate),
};
