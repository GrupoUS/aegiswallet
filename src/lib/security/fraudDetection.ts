...
export class FraudDetectionService {
	...
	async analyzeSecurityEvent(
		...
		try {
			...
			// Location Analysis
			const locationRisk = await this.analyzeLocationPatterns(
				event,
				userProfile,
			);
			...
			// Behavior Analysis
			const behaviorRisk = await this.analyzeBehaviorPatterns(
				event,
				userProfile,
			);
	...
	private async analyzeFrequencyPatterns(event: SecurityEvent): Promise<{
		...
		const { data: recentFailures } = await supabase
			...
			.gte(
				'created_at',
				new Date(Date.now() - this.config.timeWindows.short).toISOString(),
			);

		if (
			recentFailures &&
			recentFailures.length >= this.config.maxFailedAttempts
		) {
			score += 0.4;
			anomalies.push(
				`High frequency of failed attempts: ${recentFailures.length} in last hour`,
			);
		...
		if (burstAttempts && burstAttempts.length >= 10) {
			score += 0.5;
			anomalies.push(
				`Burst authentication attempts: ${burstAttempts.length} in 5 minutes`,
			);
	...
	private async analyzeLocationPatterns(
		...
		if (!knownLocation) {
			score += 0.3;
			anomalies.push(
				`New location detected: ${event.location.city}, ${event.location.country}`,
			);
		...
		for (const recentLocation of recentLocations) {
			...
			const timeDiff =
				(event.timestamp.getTime() - recentLocation.timestamp.getTime()) /
				(1000 * 60 * 60); // hours
			const speed = distance / timeDiff;

			const impossibleTravelThreshold =
				this.patterns.get('impossible_travel')?.threshold ?? 1000;
			if (speed > impossibleTravelThreshold) {
				score += 0.8;
				anomalies.push(
					`Impossible travel detected: ${speed.toFixed(0)} km/hour`,
				);
		...
		if (locationCount) {
			...
			if (uniqueLocations >= 3) {
				score += 0.4;
				anomalies.push(
					`Multiple locations detected: ${uniqueLocations} in 1 hour`,
				);
	...
	private async analyzeBehaviorPatterns(
		...
		const isUnusualTime =
			!userProfile.typicalBehavior.activeHours.includes(currentHour);
	...
	private async analyzeVelocityPatterns(event: SecurityEvent): Promise<{
		...
		if (rapidAttempts && rapidAttempts.length >= 5) {
			score += 0.3;
			anomalies.push(
				`Rapid successive attempts: ${rapidAttempts.length} in 1 minute`,
			);
	...
	private async getUserBehaviorProfile(
		...
		// TODO: Requires database migration for user_behavior_profiles table
	...
	> {
		const { data } = await supabase
			...
			.gte(
				'created_at',
				new Date(Date.now() - this.config.timeWindows.medium).toISOString(),
			)
		...
		return data
			...
			.filter(
				(
					entry,
				): entry is { latitude: number; longitude: number; timestamp: Date } =>
					entry !== null,
			);
	}
	 ...
	 */
	private async analyzeAccountTakeoverPatterns(
		event: SecurityEvent,
	): Promise<number> {
		let score = 0;
		...
		if (recentEvents && recentEvents.length >= 3) {
			...
			if (
				failuresBeforeSuccess.length >= 2 &&
				lastEvent.event_type === 'auth_success'
			) {
	...
	private parseMetadata(value: Json | null): EventMetadata | null {
		...
		if (this.isJsonRecord(record.location)) {
			...
			metadata.location = {
				city:
					typeof locationRecord.city === 'string' ? locationRecord.city : null,
				country:
					typeof locationRecord.country === 'string'
	...
	private serializeKnownDevices(
		devices: UserBehaviorProfile['knownDevices'],
	): Json {
	...
	private toJsonValue(value: unknown): Json | null {
		...
		if (Array.isArray(value)) {
			return value.map((item) => this.toJsonValue(item)) as Json;
		}

		if (typeof value === 'object') {
			const result: Record<string, Json> = {};
			for (const [key, val] of Object.entries(value)) {
				result[key] = this.toJsonValue(val) ?? null;
			}
	...
	private isJsonRecord(value: Json | null): value is Record<string, Json> {
