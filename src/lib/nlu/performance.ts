...
export class NLUPerformanceTracker {
  ...
  trackErrorRecovery(
    ...
    this.metrics.errors.averageRecoveryTime = 
      ((this.metrics.errors.averageRecoveryTime * (totalRecoveries - 1)) + recoveryTime) / totalRecoveries
  ...
  getHealthScore(): {
    ...
    factors.push({
      ...
      weight: 0.3,
      impact: processingScore < 70 ? 'High processing times detected' : 'Processing times within targets'
    })
  ...
  getRecommendations(): string[] {
    ...
    for (const [intent, accuracy] of Object.entries(metrics.accuracy.intentAccuracy)) {
      if (accuracy < 80) {
        recommendations.push(`Intent "${intent}" has low accuracy (${accuracy.toFixed(1)}%), consider adding more training examples`)
      }
    }
    ...
    for (const [region, accuracy] of Object.entries(metrics.regional.accuracyByRegion)) {
      if (accuracy < 75) {
        recommendations.push(`Regional accuracy for "${region}" is low (${accuracy.toFixed(1)}%), consider adding regional training data`)
      }
    }
  ...
  generatePerformanceReport(timeRange: 'hour' | 'day' | 'week' | 'month' = 'day'): {
    ...
    const summary = {
      ...
      successRate: metrics.processing.totalRequests > 0 ? 
        (metrics.processing.successfulRequests / metrics.processing.totalRequests) * 100 : 0,
    ...
    const trends = {
      processingTime: this.calculateTrend(this.processingTimes.slice(-10)) as 'improving' | 'stable' | 'degrading',
      accuracy: this.calculateTrend(Object.values(metrics.accuracy.intentAccuracy).slice(-5)) as 'improving' | 'stable' | 'degrading',
      errorRate: this.calculateTrend([metrics.errors.errorRate]) as 'improving' | 'stable' | 'degrading'
    }
  ...
  private startMonitoring(): void {
    ...
    if (this.config.alertingEnabled) {
      this.timers.set('alerts', setInterval(() => {
  ...
  private updateProcessingMetrics(request: any): void {
    ...
    processing.averageProcessingTime = 
      ((processing.averageProcessingTime * (processing.totalRequests - 1)) + request.processingTime) / processing.totalRequests
    ...
    if (request.region) {
      ...
      this.metrics.regional.processingTimeByRegion[request.region] = 
        ((this.metrics.regional.processingTimeByRegion[request.region] * 
          (this.metrics.regional.requestVolumeByRegion[request.region] || 0)) + request.processingTime) / 
  ...
  private updateAccuracyMetrics(request: any): void {
    ...
    accuracy.intentAccuracy[request.intent] = 
      ((accuracy.intentAccuracy[request.intent] * (totalIntentRequests - 1)) + (request.success ? 100 : 0)) / totalIntentRequests

    // Update overall accuracy
    const totalRequests = Object.values(accuracy.intentAccuracy).reduce((sum, acc) => sum + acc, 0)
    const totalCorrect = Object.values(accuracy.intentAccuracy).reduce((sum, acc) => sum + (acc * totalRequests / 100), 0)
    ...
    if (request.region) {
      ...
      this.metrics.regional.accuracyByRegion[request.region] = 
        ((this.metrics.regional.accuracyByRegion[request.region] * (regionalTotal - 1)) + (request.success ? 100 : 0)) / regionalTotal
    }
    ...
    this.metrics.temporal.accuracyByHour[hour] = 
      ((this.metrics.temporal.accuracyByHour[hour] * (this.metrics.temporal.requestVolumeByHour[hour] - 1)) + (request.success ? 100 : 0)) / 
      this.metrics.temporal.requestVolumeByHour[hour]
    ...
    this.metrics.temporal.accuracyByDayOfWeek[dayOfWeek] = 
      ((this.metrics.temporal.accuracyByDayOfWeek[dayOfWeek] * (this.metrics.temporal.requestVolumeByDayOfWeek[dayOfWeek] - 1)) + (request.success ? 100 : 0)) / 
      this.metrics.temporal.requestVolumeByDayOfWeek[dayOfWeek]
  }
  ...
  private updateErrorMetrics(request: any): void {
    if (!request.success) {
      ...
      // Update error rate
      errors.errorRate = (errors.errorRate * (errors.errorRate > 0 ? this.metrics.processing.totalRequests - 1 : 0) + 1) / this.metrics.processing.totalRequests
    }
  }
  ...
  private updateDerivedMetrics(): void {
    ...
    if (this.metrics.processing.totalRequests > 0) {
      this.metrics.errors.errorRate = (this.metrics.processing.failedRequests / this.metrics.processing.totalRequests) * 100
    }
  }
  ...
  private calculateProcessingScore(): number {
    ...
    // Success rate score
    const successRate = processing.totalRequests > 0 ? 
      (processing.successfulRequests / processing.totalRequests) * 100 : 100
    if (successRate < 95) {
  ...
  private calculateAccuracyScore(): number {
    ...
    const highConfidenceRatio = accuracy.confidenceDistribution.high / 
      (accuracy.confidenceDistribution.high + accuracy.confidenceDistribution.medium + accuracy.confidenceDistribution.low)
...
export function createNLUPerformanceTracker(config?: Partial<PerformanceConfig>): NLUPerformanceTracker {
