...
export function createInsightsTools(userId: string) {
  ...
  return {
    getSpendingAnalysis: tool({
      description: 'Analisa detalhadamente os gastos do usuário por categoria, tendências e padrões.',
      inputSchema: z.object({
        startDate: z.string().datetime().describe('Data inicial da análise'),
        endDate: z.string().datetime().describe('Data final da análise'),
        categoryIds: z.array(z.string().uuid()).optional().describe('Analisar categorias específicas'),
      ...
      execute: async ({ startDate, endDate, categoryIds, includePredictions }) => {
        try {
          ...
          if (error) {
            secureLogger.error('Erro ao buscar transações para análise', { error: error.message, userId });
    ...
    getCashFlowForecast: tool({
      ...
      inputSchema: z.object({
        ...
        confidenceLevel: z.number().min(0.5).max(0.95).default(0.8).describe('Nível de confiança da previsão'),
      }),
        ...
        try {
          ...
          for (let i = 0; i < forecastMonths; i++) {
            ...
            const monthEvents = futureEvents.filter(event => {
              ...
              return eventDate.getMonth() === forecastDate.getMonth() && 
            ...
            const confidence = pattern?.dataPoints ? Math.min(pattern.dataPoints / 12, confidenceLevel) : confidenceLevel;
          ...
          return {
            forecast,
            summary: {
              netCashFlowStatus: netCashFlow > 0 ? 'positivo' : netCashFlow < 0 ? 'negativo' : 'equilibrado',
    ...
    getAnomalyDetection: tool({
      description: 'Detecta padrões anômalos de gastos que possam indicar fraudes ou problemas financeiros.',
      inputSchema: z.object({
        analysisPeriod: z.enum(['7d', '30d', '90d']).default('30d').describe('Período de análise'),
        severityThreshold: z.enum(['low', 'medium', 'high', 'critical']).default('medium').describe('Nível mínimo de severidade'),
      ...
      execute: async ({ analysisPeriod, severityThreshold, includeRecommendations }) => {
        try {
          ...
          const highValueTransactions = txList.filter(tx => 
    ...
    getBudgetRecommendations: tool({
      ...
      inputSchema: z.object({
        targetSavingsRate: z.number().min(0.05).max(0.5).default(0.2).describe('Taxa de economia desejada (5-50%)'),
        analysisMonths: z.number().min(3).max(12).default(6).describe('Meses de histórico para análise'),
      ...
      execute: async ({ targetSavingsRate, analysisMonths, prioritizeEssential }) => {
        try {
          ...
          const categoryExpenses = new Map<string, {
            categoryName: string;
            categoryId: string;
            totalAmount: number;
            ...
            .forEach(tx => {
              ...
              if (!categoryExpenses.has(catId)) {
                categoryExpenses.set(catId, {
                  ...
                  averageMonthly: 0,
                  percentage: 0,
                  isEssential: cat.is_system || prioritizeEssential && ['Alimentação', 'Moradia', 'Transporte', 'Saúde'].includes(cat.name),
          ...
          const essentialCategories = categories.filter(cat => cat.isEssential).sort((a, b) => b.averageMonthly - a.averageMonthly);
          const nonEssentialCategories = categories.filter(cat => !cat.isEssential).sort((a, b) => b.averageMonthly - a.averageMonthly);
          ...
          const totalNonEssentialExpenses = nonEssentialCategories.reduce((sum, cat) => sum + cat.averageMonthly, 0);
          ...
          // Calcular projeções
          const projectedMonthlyExpenses = budgetRecommendations.reduce((sum, rec) => sum + rec.recommendedMonthlyBudget, 0);
...
// Helper functions
async function analyzeSpendingTrends(userId: string, startDate: Date, endDate: Date): Promise<any[]> {
...
function generateSpendingInsights(categories: CategorySpending[], totalSpending: number): string[] {
  ...
  insights.push(`${topCategory.categoryName} representa ${topCategory.percentage.toFixed(1)}% dos gastos totais`);
  ...
  const highVarianceCategories = categories.filter(cat => 
    cat.trendPercentage && Math.abs(cat.trendPercentage) > 20
  );
  
  if (highVarianceCategories.length > 0) {
    insights.push(`${highVarianceCategories.map(cat => cat.categoryName).join(', ')} mostraram variação significativa`);
  }
...
function generateSpendingRecommendations(categories: CategorySpending[], trends: any[]): string[] {
  ...
  highExpenseCategories.forEach(cat => {
    recommendations.push(`Considere revisar gastos em ${cat.categoryName} (${cat.percentage.toFixed(1)}% do total)`);
  });
  
  // Recomendações baseadas em tendências
  const increasingCategories = categories.filter(cat => cat.trend === 'increasing');
  if (increasingCategories.length > 0) {
    recommendations.push(`Atenção: ${increasingCategories.map(cat => cat.categoryName).join(', ')} estão em alta`);
  }
...
function identifyCashFlowFactors(monthlyBreakdown: MonthlyCashFlow[], patterns: any[]): string[] {
  ...
  if (expensesVariance > 1000) { // Threshold arbitrário
...
function generateCashFlowWarnings(monthlyBreakdown: MonthlyCashFlow[], netCashFlow: number): string[] {
  const warnings = [];
...
function findPotentialDuplicates(transactions: any[]): any[][] {
  ...
  amountGroups.forEach(group => {
    if (group.length >= 2) {
      ...
      if (daysDiff <= 3) { // Mesmo dia ou até 3 dias de diferença
        groups.push(group);
      }
    }
  });
...
function detectExcessiveTransactionPattern(transactions: any[]): FinancialAnomaly | null {
  ...
  if (maxTransactions > 20) { // Threshold arbitrário
    return {
...
function generateAnomalyRecommendations(anomalies: FinancialAnomaly[], riskLevel: string): string[] {
  const recommendations = [];
  ...
  if (anomalies.some(a => a.type === 'duplicate_transaction')) {
    recommendations.push('Verifique extratos bancários para confirmar duplicidade e conteste se necessário');
  }
