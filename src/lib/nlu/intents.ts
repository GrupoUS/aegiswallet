...
export const INTENT_DEFINITIONS: Record<IntentType, IntentDefinition> = {
  [IntentType.CHECK_BALANCE]: {
    ...
    patterns: [
      ...
      /\b(quanto|qual)\s+(tenho|tem|sobrou|ta|esta|está)\b/i,
  ...
  [IntentType.CHECK_BUDGET]: {
    ...
    patterns: [
      ...
      /\b(quanto|qual)\s+(falta|resta)\s+(do|no)?\s*orcamento\b/i,
    ],
  ...
  [IntentType.CHECK_INCOME]: {
    ...
    patterns: [
      ...
      /\b(recebimento|entrada|credito|salario)\s+(do\s+mes|mensal|proximo|pendente)\b/i,
      /\b(vai|vou)\s+(ter|receber|entrar)\s+quanto\b/i,
      /\b(quanto|qual)\s+(vou|vai)\s+(ganhar|receber)\b/i,
      /\b(salario|pagamento)\s+(cai|entra)\s+quando\b/i,
    ],
    keywords: [
      'receber',
      'recebimento',
  ...
  [IntentType.FINANCIAL_PROJECTION]: {
    ...
    patterns: [
      /\b(projecao|previsao|estimativa)\s+(financeira|do\s+mes|mensal|anual)\b/i,
      /\b(como|qual)\s+(vai|fica|esta|está)\s+(meu|o)?\s*(mes|ano|semana)\b/i,
      /\b(vou|vai)\s+(sobrar|faltar|ter)\s+quanto\b/i,
      /\b(quanto|qual)\s+(sobra|falta|resta)\s+(no\s+fim|final)\s+(do\s+mes|mes)\b/i,
      /\b(balanco|resultado)\s+(do\s+mes|mensal|previsto)\b/i,
    ],
    keywords: [
      'projeção',
      'projecao',
